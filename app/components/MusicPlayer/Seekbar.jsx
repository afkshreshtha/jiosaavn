...
const Seekbar = ({ value, min, max, onInput, setSeekTime, appTime }) => {
  ...
  return (
    <div className=" sm:flex flex-row items-center">
      ...
      <p className="hidden text-white sm:flex flex-row items-center">{value === 0 ? '0:00' : getTime(value)}</p>
      <input
        type="range"
        step="any"
        value={value}
      ...
      />
      <p className="hidden text-white sm:flex flex-row items-center">{max === 0 ? '0:00' : getTime(max)}</p>
